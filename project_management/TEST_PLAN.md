...
| Value too high | `"lives: 999"` | Clear error message, set defaults values and start the game | ❌ |
...
| Commentary | `# commentary` | Ignore commentary and start the game | ❌ |
...
| Scenario | Expected result | Status |
| -------- | --------------- | ------ |
| Move up | Pac-man move | ❌ |
| Move down | Pac-man move | ❌ |
| Move to the right | Pac-man move | ❌ |
| Move to the left | Pac-man move | ❌ |
...
| Eat a ghost | Sccore +(config value) | ❌ |
|All pacgums are eaten | Next level or game victory if it was the last level | ❌ |
| Keep score between levels | Score on level N is the same on the beginin of level N+1 | ❌ |
| Keep lives between levels | Lives on level N are the same on the beginin of level N+1 | ❌ |
